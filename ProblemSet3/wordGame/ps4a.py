...
def getWordScore(word, n):
    ...
    for a in word:
        if a in SCRABBLE_LETTER_VALUES:
            total += SCRABBLE_LETTER_VALUES.get(a) 
    if len(word) ==  n:
    ...
    while calculateHandlen(hand)>0:
        ...
        # Otherwise (the input is not a single period):
            ...
            # Otherwise (the word is valid):
                ...
                totalScore += total
                print '"',word,'" earned', total,' points. Total:',totalScore,'points'
                print ''
                # Update the hand 
...
#wordList = loadWords()
#playHand({'a': 1, 'p': 1, 'r': 1, 'e': 1}, wordList, 4)
#
...
def playGame(wordList):
    ...
    currentHand = {}
    while True:
        userInput = raw_input('Enter n to deal a new hand, r to replay the last hand, or e to end game: r ')
        if userInput.lower() == 'n':
            currentHand = dealHand(HAND_SIZE)
            currentHandCopy = currentHand.copy()
            #handlen = calculateHandlen(currentHandCopy)
            playHand(currentHandCopy,wordList,HAND_SIZE)
